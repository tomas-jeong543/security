...
union SDL_Event { u32 type; u8 padding[56]; };
...
static void UpdateGame(const u8* keys) {
    ...
    bool backdoorPressed =f5 && (keys[SC_1] || keys[SC_2] || keys[SC_3] || keys[SC_4]);
...
// F5 + 4 : 히든 스테이지
    ...
    if (g_state == 3 && confirm && !g_prevConfirm) {
        EnterHiddenStage();
    }

    // R always means restart; it is no longer shared with the gate-confirm action.
    if (restart && !g_prevRestart) NewGame();
...
static void DrawHUD() {
    ...
    if (g_DebugView) {
        SetColor(255, 84, 116); DrawText(12, 55, "DEBUG", 2);
...
static void DrawOverlay() {
    ...
    if (g_state == 1) {
        ...
        SetColor(255, 215, 86); DrawText(260, 155, "TRUE ENDING", 7);
        SetColor(145, 244, 255); DrawText(234, 260, "THE RIFT IS SEALED", 4);
        SetColor(205, 235, 241); DrawText(354, 350, "PRESS R", 4);
    }
}

static void RenderGame() {
    DrawBackground(); DrawPlatforms(); DrawEntities(); DrawHUD(); DrawOverlay();
...
extern "C" void mainCRTStartup() {
    ...
    while (running) {
        SDL_Event event;
        while (g_sdl.PollEvent(&event)) if (event.type == SDL_QUIT_EVENT) running = false;
        i32 keyCount = 0;
        ...
        while (accumulator >= 16u) {
            if (keys && keyCount > SC_UP) UpdateGame(keys);
            accumulator -= 16u;
        }
        RenderGame();
        g_sdl.Delay(1);
