...
[상세 분석 보고서](리버싱프로젝트4_v2.pdf)

상세 보고서는 최종 결과뿐만 아니라 각 변수와 함수의 역할을 어떻게 추론하고 정적 분석 결과를 동적 분석으로 검증했는지를 단계별로 기록하고 있습니다.
## 저장소 파일 안내와 작성 범위

- [상세 분석 보고서](리버싱프로젝트4_v2.pdf)
- [Python 분석 도구 설명](source/코드_통합설명본.txt): 기존 문서에서 직접 재구현했다고 설명한 분석 도구입니다. 파일별 외부 코드 활용 범위는 확인 필요입니다.
- [분석 도구 소스](source/): 분석 대상 실행 파일의 원본 소스가 아니라 알고리즘을 재구현한 Python 코드입니다.
- [실행 파일 구분](exe_files/explanation.txt): 원본 분석 대상과 출력 수정·패치본을 구분합니다. 실행 파일의 제작 주체와 확보 방법은 확인 필요입니다.

코드 설명의 명령은 `source/`를 작업 디렉터리로 삼습니다. 실행 파일 인자에는 실제 대상 파일의 경로를 지정하십시오. `exe_files/` 아래 실행 파일이 로컬에 있더라도 Git에서 제외되어 있을 수 있습니다. 도구 버전과 새로 내려받은 환경에서의 재현 절차는 TODO입니다.
